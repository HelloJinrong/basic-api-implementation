...
class RsListApplicationTests {
    ...
    public void should_throw_method_argument_not_valid_exception()throws Exception{
        ObjectMapper objectMapper = new ObjectMapper();
        User user = new User("annieaaaaa","female",20,"a@b.com","17777777777");
        RsEvent rsEvent = new RsEvent("猪肉涨价啦", "经济", user);
        String jsonString = objectMapper.writeValueAsString(rsEvent);
    ...
    public void should_throw_exception_get_list_between() throws Exception {
        mockMvc.perform(get("/rs/list?start=-1&end=12"))
                .andExpect(status().isBadRequest())
    ...
    public void throw_exception_when_eventName_null() throws Exception {
        String jsonString = "{\"eventName\":null,\"keyWord\":\"电影\",\"user\": {\"name\":\"hjr\"," +
                "\"age\": 18,\"gender\": \"female\",\"email\": \"hjr@b.com\",\"phone\": \"12345678901\"}}";
        mockMvc.perform(post("/rs/event").content(jsonString).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error", is("invalid param")));
    }
    ...
    public void throw_exception_when_keyWord_null() throws Exception {
        String jsonString = "{\"eventName\":\"abc\",\"keyWord\":null,\"user\": {\"name\":\"hjr2\"," +
                "\"age\": 15,\"gender\": \"female\",\"email\": \"hjr@b.com\",\"phone\": \"12345678901\"}}";
        mockMvc.perform(post("/rs/event").content(jsonString).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error", is("message error!")));
    }

    @Test
    public void throw_exception_when_user_null() throws Exception {
        String jsonString = "{\"eventName\":\"abc\",\"keyWord\":\"电影\",\"user\": {\"name\":null," +
                "\"age\": 15,\"gender\": \"female\",\"email\": \"hjr@b.com\",\"phone\": \"12345678901\"}}";
        mockMvc.perform(post("/rs/event").content(jsonString).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error", is("message error!")));
